...
void test7() 
{
	// Allocate more pages than it could be allocated
	try {
		for (i = 0; i < num+1; i++) {
			bufMgr->allocPage(file1ptr, pid[i], page);
			sprintf((char*)tmpbuf, "test.1 Page %d %7.1f", pid[i], (float)pid[i]);
		...
		PRINT_ERROR("ERROR :: There is no room for the pages. Exception should have been thrown before execution reaches this point.");
	}
	catch (BufferExceededException e) 
	{
	}
	...
	for (i = 0; i < num; i++) {
		...
		bufMgr->unPinPage(file1ptr, pid[i], false); // Page pin count should be 2
		bufMgr->unPinPage(file1ptr, pid[i], false);
	}
...
void test8() 
{
	...
	// discard a specific page
	bufMgr->disposePage(file1ptr, pid[0]);
	try {
		// try to read this page
		bufMgr->readPage(file1ptr, pid[0], page);
		PRINT_ERROR("ERROR :: InvalidPageException should have been thrown before reaches this point.");
	}
	catch (InvalidPageException e) {
